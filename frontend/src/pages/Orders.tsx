...
export default function OrdersPage() {
  ...
  const [closed, setClosedState] = useState<'' | 'active' | 'success' | 'fail'>(
    () => (localStorage.getItem('omega_orders_closed') as '' | 'active' | 'success' | 'fail') || '',
  )
  const setClosed = (v: '' | 'active' | 'success' | 'fail') => {
    setClosedState(v)
  ...
  const { data, isLoading } = useQuery({
    ...
    enabled: trash ? true : closed === '' || closed === 'active' || (closed === 'success' && !!successStageId) || (closed === 'fail' && !!failStageId),
    queryFn: async () => {
      if (trash) {
